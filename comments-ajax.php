...
if ( empty($post->comment_status) ) {
	...
	err(__( 'Invalid comment status.' , 'PhotoBroad' )); // 將 exit 改為錯誤提示
}
...
if ( !comments_open($comment_post_ID) ) {
	...
	err(__('Sorry, comments are closed for this item.','PhotoBroad')); // 將 wp_die 改為錯誤提示
} elseif ( 'trash' == $status ) {
	...
	err(__('Password Protected','PhotoBroad')); // 將 exit 改為錯誤提示
} else {
...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status )
		err(__('Sorry, you must be logged in to post a comment.','PhotoBroad')); // 將 wp_die 改為錯誤提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('Error: please fill the required fields (name, email).','PhotoBroad') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('Error: please enter a valid email address.','PhotoBroad') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('Error: please type a comment.','PhotoBroad') ); // 將 wp_die 改為錯誤提示
...
if ( $wpdb->get_var($dupe) ) {
    err(__('Duplicate comment detected; it looks as though you&#8217;ve already said that!','PhotoBroad'));
}
...
if ( $flood_die ) {
    err(__('You are posting comments too quickly.  Slow down.','PhotoBroad'));
	}
}
